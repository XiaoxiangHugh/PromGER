...
def process_data(pos_train_fa, neg_train_fa, test_fa, shuffle_seed=5, validate_size=None, validate_random_seed=5):
    ...
    train_validate_df = pd.concat([P_df, N_df], axis=0) #concat沿着指定的轴将多个DataFrame拼接到一起。axis=0,表拼接方式是上下堆叠

    if shuffle_seed is not None: #设定shuffle_seed=5
        # Shuffule the training set data. 打乱训练集数据
...
def read_fa(path):
    records = list(SeqIO.parse(path, format="fasta"))#读取序列文件，生成SeqRecord对象。
    id_seq_dict = {} #创造序列文件的字典
    for x in records:
        id = str(x.id)
        seq = "".join([x for x in str(x.seq)]) # seq = "".join([x for x in str(x.seq).replace("U", "T") if x in "ATCG"])
        id_seq_dict[id] = seq
    return id_seq_dict


def seq_dicts2df(seq_dicts, label):
    df = pd.DataFrame() #DataFrame是一种表格型数据结构，含有一组有序的列，每列可以是不同的值
